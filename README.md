...
10 道选择题，覆盖 Python 核心知识点，题目存于数据库、可在管理后台维护。纯代码评分（非 AI），按可配置的评分规则 0-100 分判定等级并推荐学习起点。默认 0-30 入门、31-60 入门（可跳过基础）、61-80 进阶、81-100 高级（对应 beginner/intermediate/advanced）；分档与推荐起点可在管理后台「诊断题管理 → 评分规则」配置（存于 `system_settings` 表的 `diagnostic_scoring_rules` 键），保存后即时生效。诊断结果存入数据库并在前端展示。
