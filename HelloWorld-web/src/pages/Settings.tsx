...
export default function Settings() {
  ...
  const [editorFontSize, setEditorFontSize] = useState(() => loadSetting<number>('helloworld_editor_font_size', 14))
  const [reminderEnabled, setReminderEnabled] = useState(() => loadSetting<boolean>('helloworld_reminder', true))
  const [achievementNotify, setAchievementNotify] = useState(() => loadSetting<boolean>('helloworld_achievement_notify', true))
  ...
  const handleClearAIHistory = useCallback(() => {
    localStorage.removeItem('helloworld_ai_history'); showToast('AI 对话历史已清空')
  }, [showToast])
  const handleExportData = useCallback(() => {
    const data: Record<string, unknown> = {}
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i)
      if (key && key.startsWith('helloworld_')) {
        try { data[key] = JSON.parse(localStorage.getItem(key)!) } catch { data[key] = localStorage.getItem(key) }
      }
    }
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' })
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'HelloWorld-backup.json'; a.click()
        ...
        <div className="max-w-3xl mx-auto px-6 py-8 space-y-5">
          ...
          <section style={sectionStyle}>
            ...
            <div>
              ...
              <input type="range" min={10} max={24} step={1} value={editorFontSize}
                onChange={(e) => { const v = Number(e.target.value); setEditorFontSize(v); saveSetting('helloworld_editor_font_size', v); showToast(`代码字号 → ${v}px`) }}
          ...
          <section style={sectionStyle}>
            ...
            <div className="space-y-4">
              <ToggleRow label="学习提醒" description="每日打卡提醒，保持学习节奏" enabled={reminderEnabled}
                onToggle={() => { const v = !reminderEnabled; setReminderEnabled(v); saveSetting('helloworld_reminder', v); showToast(v ? '学习提醒已开启' : '学习提醒已关闭') }} />
              <ToggleRow label="成就解锁通知" description="解锁成就时显示动画和提示" enabled={achievementNotify}
                onToggle={() => { const v = !achievementNotify; setAchievementNotify(v); saveSetting('helloworld_achievement_notify', v); showToast(v ? '成就通知已开启' : '成就通知已关闭') }} />
            </div>
          </section>
          ...
          <section style={sectionStyle}>
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => { if (window.confirm('确定要重置所有学习进度吗？此操作不可恢复。')) { const keys: string[] = []; for (let i = 0; i < localStorage.length; i++) { const key = localStorage.key(i); if (key && (key.startsWith('helloworld_progress') || key.startsWith('helloworld_xp'))) keys.push(key) } keys.forEach(k => localStorage.removeItem(k)); showToast('学习进度已重置，请刷新页面') } }}
